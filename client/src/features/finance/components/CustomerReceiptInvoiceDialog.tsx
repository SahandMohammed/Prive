...
import type { OutstandingSalesInvoice } from '../types/finance.types'

interface CustomerReceiptInvoiceDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  customerName: string
  currencyCode: string
  invoices: OutstandingSalesInvoice[]
  currentAllocations: { salesInvoiceId: string; amount: number }[]
  onApply: (
...
export function CustomerReceiptInvoiceDialog({
  open,
  onOpenChange,
  customerName,
  currencyCode,
  ...
  currentAllocations,
  onApply,
}: CustomerReceiptInvoiceDialogProps) {
  // Map of salesInvoiceId -> allocation amount
  ...
  useEffect(() => {
    if (open) {
      ...
      currentAllocations.forEach((alloc) => {
        if (alloc.amount > 0) {
          initialMap[alloc.salesInvoiceId] = alloc.amount
        }
      })
  ...
  const handleToggleInvoice = (invoice: OutstandingSalesInvoice) => {
  ...
  const handleAmountChange = (invoice: OutstandingSalesInvoice, val: number) => {
  ...
  // Select all (settle full outstanding for all available invoices)
      ...
      <DialogContent className="max-w-4xl">
        <DialogHeader>
          <div className="flex items-center gap-2">
            ...
            <DialogTitle className="text-lg font-bold text-slate-900 dark:text-slate-100">
              Choose Sales Invoices to Settle
            </DialogTitle>
          </div>
          <DialogDescription className="text-xs text-slate-500">
            Select outstanding receivables for <strong>{customerName}</strong> in{' '}
            <strong>{currencyCode}</strong>. Selected amounts will auto-calculate the total receipt.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {allSelected ? (
                ...
                <>
                  <CheckSquare className="size-3.5 text-primary" /> Settle All in Full
                </>
              )}
            </Button>
              ...
              {invoices.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={7} className="h-32 text-center text-xs text-slate-400">
                    No outstanding sales invoices found for this customer in {currencyCode}.
                  </TableCell>
                </TableRow>
              ) : (
        ...
        <div className="flex flex-col gap-3 rounded-lg border border-slate-200 bg-slate-50/80 p-3 sm:flex-row sm:items-center sm:justify-between dark:border-slate-800 dark:bg-slate-800/40">
          <div className="flex items-center gap-4 text-xs">
            <span className="text-slate-600 dark:text-slate-400">
              ...
              <strong className="text-slate-900 dark:text-slate-100">
                {selectedCount} of {invoices.length} invoices
              </strong>
            </span>
          ...
          <div className="flex items-center gap-2 text-right">
            <span className="text-xs text-slate-500">Total Selected to Settle:</span>
        ...
        <DialogFooter className="mt-2">
          ...
          <Button
            type="button"
            size="sm"
            className="gap-1.5 bg-[#e05d38] font-medium text-white hover:bg-[#c94f2d]"
