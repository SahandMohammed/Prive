...
export const openingStockSchema = z.object({ branchId: requiredId, warehouseId: requiredId, documentDate: z.string().min(1, 'Date is required'), notes: z.string(), lines: z.array(z.object({ productId: requiredId, unitOfMeasureId: requiredId, quantity: z.number().positive('Quantity must be positive'), unitCost: z.number().min(0, 'Cost cannot be negative') })).min(1, 'Add at least one product') })
