...
export function PurchaseInvoicesPage() {
  ...
  return <div className="flex h-full flex-col space-y-6">
    ...
    <div className="grid gap-3 md:grid-cols-4">
      ...
      <Select value={branchId} onChange={(event) => { setBranchId(event.target.value); setWarehouseId(''); resetPage() }}><option value="">All branches</option>{branches.map((item) => <option key={item.id} value={item.id}>{item.code} — {item.name}</option>)}</Select>
